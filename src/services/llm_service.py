...
class LLMService(ABC):
    ...
    def _build_prompt(self, question: str, schema: str, db_type: str) -> str:
        return f"""Você é um especialista em SQL.
                    Converta a pergunta do usuário em uma consulta SQL válida baseada no schema e no banco informado.

                    SCHEMA (formato: tabela(coluna:tipo, ...)):
                    {schema}
                    ...
                    REGRAS:
                    1. Retorne APENAS o código SQL puro, sem markdown, sem ```sql.
                    2. Use somente colunas existentes no schema acima.
                    3. Apenas SELECT é permitido.
                    4. Se a pergunta pedir alteração de dados, responda: ERRO: operação não permitida.
                    5. Se houver colunas com mesmo nome em tabelas diferentes, use aliases AS."""
...
class GeminiLLMService(LLMService):
    ...
    def explain_results(self, question: str, results: str) -> str:
        prompt = f"""Pergunta original: {question}
                     ...
                     Não repita a pergunta nem os dados — foque na interpretação."""
